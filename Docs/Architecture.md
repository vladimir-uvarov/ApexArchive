...
- Tests mirror their module, with dedicated test doubles and test-only resources. `ApexArchiveTests` runs inside the app as its host and covers what only the app target can break: asset naming conventions, bundled documents, and hand-written catalogs.
...
Unit tests focus on behavior: validation, cancellation and ordering, offline recovery, saved-state consistency, sorting, photo permissions and observation isolation. Project validation checks stale source references, missing package associations and Debug architecture alignment with Swift packages. Source-structure checks also fail on an app `View` no other file references, because two unreachable screens shipped before that check existed. Localization validation compares every catalog value with its source default, since the catalog wins at runtime and a default edited only in Swift silently keeps the old copy. CI builds the complete iOS app and assets after package tests and strict formatting/lint checks. See `VALIDATION.md` for what has actually run locally and what still requires simulator verification.
...
Visible and preloaded requests share pending work by title; the data decorator also shares a single task by image URL. The shared task includes the disk lookup to avoid a race between a cache miss and a completed concurrent download. Prefetching considers at most twenty-four unique titles per batch and still runs two workers per batch, so a wider batch does not mean more concurrent requests. A card that has already shown a photo re-requests it if the bounded cache evicted it while the card stayed on screen. Disappearing callers stop scheduling additional preloads, while an already shared request may complete for other views. The garage uses LazyVStack to avoid instantiating every image view at once.

`MotionTokens` owns launch and interaction timings. The launch treatment uses the approved local artwork and SwiftUI, without an animation dependency. The system launch screen, full-screen SwiftUI treatment, and app background share the adaptive LaunchBackground color asset: white in light appearance and deep navy in dark appearance. Splash text uses the primary foreground style, and its icon shadow is softer in light appearance. The overlay dismisses once the initial archive or an error is available, allowing 550 ms for its reveal, or immediately with Reduce Motion. It does not wait for remote images. Horizontal filter, detail-section and discovery activity rails can draw beyond their inset bounds so scrolling is not clipped at the page padding. Photo reveal, filter selection, and bookmark replacement are deliberately restrained; the full driver grid is not animated through potentially hundreds of reordered cells. `CardPressStyle` gives every card a small scale under the finger so a tap is answered before navigation starts, and Reduce Motion removes that scale rather than shortening it. Driver and car cards declare a zoom transition into their detail screen; the effect requires iOS 18, and `zoomSource`/`zoomDestination` fall back to the standard push on iOS 17 so no call site branches. Horizontal chip rails fix their cross axis, because a scroll view is greedy on both axes and the chips otherwise drift vertically.
...
The animated splash reads the installed icon from SettingsViewModel's `installedIcon`, which asks iOS on demand. The selection cached at init is unreliable for the first frame: the object graph is built before UIKit reports an alternate icon, and an earlier version of this document claimed otherwise while the splash showed the wrong helmet. Each helmet has matching full-resolution local splash artwork. The system launch background remains appearance-aware and artwork-free, avoiding a flash of the original helmet before a selected variant appears.
