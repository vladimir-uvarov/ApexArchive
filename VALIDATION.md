...
17 September 2026 · Xcode 26.2 / Swift 6.2.3 · iPhone 17 Pro simulator (iOS 26).

## Current

- 151 Swift Package Manager tests and 14 app-hosted `ApexArchiveTests` passed, zero failures. Package tests run without network access; app tests run inside the built app on the simulator.
- swift-format strict, source-structure/header checks, view-reachability check, localization key and value checks, and SwiftLint 0.65.1: zero violations across 212 Swift files.
- Full `xcodebuild test` on the simulator succeeded; CI runs the same command.
- Live checks against F1DB and Wikimedia (run separately, not in CI): 917 drivers; all 42 curated drivers and all 55 titled cars resolved a licensed photo; a 40-driver sample of the historical set resolved 28 after the disambiguation retry, the rest having no free-licensed image.
- Simulator screenshots verified: pinned search and filters on Drivers in light and dark appearance, filter change while scrolled, splash helmet matching the installed alternate icon, all three Discover spotlights, Places list, circuit outline sizing.
- Not verified here: gestures (no simulator gesture API) — the filter-rail interaction was confirmed from a screen recording supplied by the author; device performance, Dynamic Type at the largest sizes, VoiceOver end-to-end.

## Reproduce

`Scripts/format.sh`, `Scripts/lint.sh`, `Scripts/test.sh`, then `xcodebuild test -project ApexArchive.xcodeproj -scheme ApexArchive -destination 'platform=iOS Simulator,name=iPhone 17'`. Use `swift run --package-path Packages/ArchiveKit ArchiveIntegrationCheck --live` to contact external services.

---

## Earlier record (14 September 2026)
