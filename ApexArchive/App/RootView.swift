...
struct RootView: View {
    ...
    var body: some View {
        ...
        .overlay {
            if showingLaunch { LaunchScreenView(icon: dependencies.settings.selectedIcon).transition(.opacity) }
        }
        ...
        .task { await archive.loadIfNeeded() }
        .task(id: archive.snapshot.drivers.isEmpty) {
            ...
            let titles =
                [featured].compactMap { $0 } + snapshot.cars.prefix(2).compactMap(\.wikipediaTitle)
                + snapshot.drivers.prefix(6).compactMap(\.wikipediaTitle)
